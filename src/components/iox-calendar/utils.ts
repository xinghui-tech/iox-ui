...
export function getDayByOffset(date: number | Date, offset: number) {
  date = new Date(date);
...
export function calcDateNum(date: [Date | number, Date | number]) {
  const day1 = new Date(date[0]).getTime();
  const day2 = new Date(date[1]).getTime();
  return (day2 - day1) / (1000 * 60 * 60 * 24) + 1;
}

export function copyDates(dates: number | Date | (number | Date | null)[]) {
